...
import CharacterFactory, { BuildSlimeOptions } from "../src/characters/character_factory";
...
class StartingScene extends Phaser.Scene implements Scene {
    ...
    create() {
        ...
        let params: BuildSlimeOptions = { slimeType: 0 };
        for (let i = 0; i < 30; i++) {
    ...
    */
    ...
    pixelsToTiles(pixels: { x: number, y: number }): Phaser.Math.Vector2 {
        return new Phaser.Math.Vector2(Math.floor(pixels.x / this.tileSize), Math.floor(pixels.y / this.tileSize));
    }
}
