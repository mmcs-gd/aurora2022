...
export class Pursuit implements Steering {
	...
	calculateImpulse() {
		...
		const target = {x:this.targetsrc.body.position.x, y:this.targetsrc.body.position.y};
		const targetDirection = {x:this.targetsrc.body.velocity.x, y:this.targetsrc.body.velocity.y};
		...
		{
			return new GoToPoint(this.owner, {x:target.x, y:target.y}, this.force)
			.calculateImpulse(); 
		}
		...
		const predictTime =
				toTarget.length() / (targetSpeed + ownerSpeed);
